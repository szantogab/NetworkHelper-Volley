 ...
 */
public class ParsedAsyncRequestFuture<T> implements Future<T>, Response.Listener<ParsedResponse<T>>, Response.ErrorListener {
    protected ParserRequest<T> mRequest;
    protected boolean mResultReceived = false;
    protected ParsedResponse<T> mResult;
    ...
    public void setListeners(Response.Listener<T> listener, Response.ErrorListener errorListener) {
        ...
        {
            if (mResult != null)
                listener.onResponse(mResult.getParsedResponse());
            else if (mException != null)
                errorListener.onErrorResponse(mException);
        }
    }
    ...
    protected synchronized T doGet(Long timeoutMs)
            throws InterruptedException, ExecutionException, TimeoutException {
        if (mException != null) {
        ...
        if (mResultReceived) {
            return mResult.getParsedResponse();
        }
    ...
    public synchronized void onResponse(ParsedResponse<T> response) {
        ...
        if (mListener != null)
            mListener.onResponse(response.getParsedResponse());
    }
